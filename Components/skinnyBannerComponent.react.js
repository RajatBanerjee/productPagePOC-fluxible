...
var SkinnyBannerComponent = React.createClass({
			getInitialState: function() {
				//console.log("promodata initial state=>",this.props.promoData);
				if (this.props.promoData) {
						return{ promoData: this.props.promoData}
				} else {
				...
				if (this.state.promoData) {
					return ( < div className = "skinny-banner-container" >
						...
						< div className = "banner-text" > {
							data.DiscountOffer
						} < /div>  < div className = "banner-details" >
						< div className = "promo-code" > Use code: < br > < /br>  < span > lalalal == {
							data.DiscountCode
						} < /span></div >
						< /div> <div className = "banner-exp-date banner-exp-date1">
						Ends: {
							data.DiscountExpirationDt
						} < /div> < div className = "banner-terms-link" > details < /div>  < div className = "clear" >
						< /div></div > < /div>
					);
				} else {
...
<<<<<<< HEAD:Components/skinnyBannerComponent.react.js
			}
		}.bind(this));
	},
	...
	render: function() {
		...
		if (this.state.promoData) {
	     return (<div className = "skinny-banner-container">
			...
			<span>  {data.DiscountCode} </span></div> 
			</div> <div className = "banner-exp-date banner-exp-date1">
			 Ends: {data.DiscountExpirationDt} 
			 </div>
			 <div onClick={this.handleClick} className = "banner-terms-link" > details </div> 
			 <div className = "clear">
			 </div></div></div>
		);
	    } else {
...
=======
		...
		module.exports = SkinnyBannerComponent;
>>>>>>> dczarnota/master:Components/skinnyBannerComponent.react.jsx
